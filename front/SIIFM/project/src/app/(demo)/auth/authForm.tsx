...
//       router.push("/"); // ou "/dashboard" dependendo da página inicial
//     }
//   }, [router]);
...
export function AuthPage() {
  ...
  useEffect(() => {
    ...
    if (token && user) {
      const parsedUser = JSON.parse(user);
      setUserData(parsedUser);
      router.push("/home"); // redireciona automaticamente
    }
  }, [router]);
  ...
  // Callback quando registro é bem-sucedido
  const handleRegisterSuccess = (success: boolean, message?: string) => {
    if (success) {
      showToast("success", message || "Cadastro realizado");
