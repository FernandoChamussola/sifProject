      ...
      <div className="min-h-screen flex flex-col items-center p-6 bg-gray-50 dark:bg-gray-900 space-y-6">
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="p-6 bg-white dark:bg-gray-800 shadow-lg rounded-2xl border border-indigo-200 dark:border-indigo-700">
            ...
            <CardContent className="text-2xl font-bold">{dashboard.totalUsuarios}</CardContent>
          </Card>
          <Card className="p-6 bg-white dark:bg-gray-800 shadow-lg rounded-2xl border border-indigo-200 dark:border-indigo-700">
            <CardTitle>Pagamentos</CardTitle>
            <CardContent className="text-2xl font-bold">{dashboard.totalPagamentos}</CardContent>
          </Card>
          <Card className="p-6 bg-white dark:bg-gray-800 shadow-lg rounded-2xl border border-indigo-200 dark:border-indigo-700">
            <CardTitle>Total Arrecadado</CardTitle>
            <CardContent className="text-2xl font-bold">{dashboard.totalArrecadado.toLocaleString()} MZN</CardContent>
          </Card>
          <Card className="p-6 bg-white dark:bg-gray-800 shadow-lg rounded-2xl border border-indigo-200 dark:border-indigo-700">
            <CardTitle>Usuários Ativos</CardTitle>
            <CardContent className="text-2xl font-bold">{dashboard.usuariosAtivos}</CardContent>
          </Card>
        </div>
        ...
        <Card className="w-full max-w-6xl p-6 bg-white dark:bg-gray-800 shadow-2xl rounded-3xl border border-indigo-200 dark:border-indigo-700">
          ...
          <CardContent className="space-y-4">
            {pagamentos.length === 0 && (
              <p className="text-center text-gray-700 dark:text-gray-200">Nenhum pagamento recente.</p>
            )}
            {pagamentos.map(p => (
              <div key={p.id} className="flex flex-col md:flex-row justify-between items-start md:items-center bg-indigo-50 dark:bg-indigo-900 rounded-xl p-4 md:p-6 shadow-md border border-indigo-200 dark:border-indigo-700">
