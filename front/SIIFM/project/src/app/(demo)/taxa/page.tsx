...
export default function GestaoTaxas() {
  ...
  // Buscar taxas da API usando Axios
  useEffect(() => {
    async function fetchTaxas() {
      try {
  ...
  async function handleAddTaxa(e: React.FormEvent) {
    ...
    try {
      const { data } = await api.post<{ success: boolean; data: Taxa; message?: string }>("/taxas", {
        nome: novaTaxa.nome,
        valor: parseFloat(novaTaxa.valor),
        periodicidade: novaTaxa.periodicidade,
      });
  ...
  async function handleSave(id: number) {
    if (!taxaEditada.nome || !taxaEditada.valor || !taxaEditada.periodicidade) return;

    try {
      const { data } = await api.put<{ success: boolean; data: Taxa; message?: string }>(`/taxas/${id}`, {
      ...
      if (data.success) {
        setTaxas(
          taxas.map((t) => (t.id === id ? data.data : t))
        );
        setEditando(null);
      } else {
        alert(data.message || "Erro ao atualizar taxa");
      }
    } catch (err) {
      console.error(err);
  ...
  // Excluir taxa com API
  async function handleDelete(id: number) {
    try {
      const { data } = await api.delete<{ success: boolean; message?: string }>(`/taxas/${id}`);
      if (data.success) {
        setTaxas(taxas.filter((t) => t.id !== id));
      } else {
        alert(data.message || "Erro ao excluir taxa");
      }
    } catch (err) {
      console.error(err);
